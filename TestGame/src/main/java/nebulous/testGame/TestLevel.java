...
public class TestLevel extends Level {
	...
	public void update(Game game, double delta) {
		...
		if (block.getPosition().x > 8)
			moveRight = false;
		if (block.getPosition().x < 4)
			moveRight = true;
		if (moveRight)
			block.move((float) (8f * delta), 0);
		else
			block.move((float) (-8f * delta), 0);
		
		if (block4.getPosition().y > 12)
			moveUp = false;
		if (block4.getPosition().y < 6)
			moveUp = true;
		if (moveUp)
			block4.move(0, (float) (8f * delta));
		else
			block4.move(0, (float) (-8f * delta));
		...
		if (Input.isKeyHeld(Input.KEY_LEFT_SHIFT)) {
			Vector3f pos;
			Vector2f tmp;
			if (camera.getPerspective() == Camera.ORTHOGRAPHIC) { // ORTHOGRAPHIC
																	// SUPPORT
